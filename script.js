...
const STORAGE_KEY = 'porquinho_inteligente_state';
...
function loadState() {
  try {
    ...
    state.extraIncome = parsed.extraIncome || 0;
    state.fixedExpenses = Array.isArray(parsed.fixedExpenses) ? parsed.fixedExpenses : [];
    state.variableExpenses = Array.isArray(parsed.variableExpenses) ? parsed.variableExpenses : [];
  } catch (error) {
    console.warn('Falha ao carregar dados:', error);
  }
}
...
function getTotals() {
  ...
  const totalVariable = state.variableExpenses.reduce((sum, item) => sum + item.value, 0);
  const totalIncome = state.salary + state.extraIncome;
  const netSavings = Math.max(totalIncome - totalFixed - totalVariable, 0);
  const savingsPercent = totalIncome > 0 ? (netSavings / totalIncome) * 100 : 0;
  return { totalFixed, totalVariable, totalIncome, netSavings, savingsPercent };
}
...
function renderTables() {
  ...
  state.fixedExpenses.forEach((item, index) => {
    const row = document.createElement('tr');
    row.innerHTML = `
      <td>${item.name}</td>
      <td>${formatCurrency(item.value)}</td>
  ...
  state.variableExpenses.forEach((item, index) => {
    const row = document.createElement('tr');
    row.innerHTML = `
      <td>${item.name}</td>
      <td>${formatCurrency(item.value)}</td>
...
function updateInputs() {
  ...
  extraInput.value = state.extraIncome || '';
}

function updateState() {
  state.salary = Number(salaryInput.value) || 0;
  state.extraIncome = Number(extraInput.value) || 0;
  saveState();
  renderSummary();
}
...
function addExpense(type) {
  ...
  if (type === 'fixed') {
    ...
    state.variableExpenses.push(item);
  }
...
function removeExpense(type, index) {
  if (type === 'fixed') {
    ...
    state.variableExpenses.splice(index, 1);
  }
  saveState();
  renderTables();
  renderSummary();
